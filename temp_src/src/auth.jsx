    ...
    <div className="login-screen">
      <div className="login-left">
        <div className="brand" style={{ gap: 4, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', marginBottom: 'var(--login-brand-mb, 24px)', marginTop: 'var(--login-brand-mt, 12px)' }}>
          <img src={LogoWhite} alt="Logo Thuê Xe Nhanh" style={{ width: '100%', maxWidth: 'var(--login-logo-max-width, 220px)', height: 'auto', objectFit: 'contain' }} />
...
function AccountSettingsScreen({ user, onClose, onSave, cars, onToggleFavorite, onAdmin }) {
  ...
  const handleAvatarFile = async (e) => {
    ...
    try {
      ...
      const optimizedInfo = await optimizeImage(file);
      const dataUrl = optimizedInfo.url;
      
      // Dùng API của ImgBB
      const apiKey = import.meta.env.VITE_IMGBB_API_KEY;
      if (!apiKey) throw new Error("Thiếu cấu hình ImgBB API Key");
      ...
      if (!data.success) throw new Error("Upload ImgBB thất bại");
            ...
            <div style={{ padding: 16, background: 'linear-gradient(135deg, var(--m-primary) 0%, #1e3a8a 100%)', borderRadius: 12, color: '#fff', textAlign: 'left', display: 'flex', flexDirection: 'column', gap: 12, boxShadow: '0 4px 12px rgba(10, 49, 97, 0.2)' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 6, fontWeight: 600, fontSize: 14 }}>
                  <BadgeCheck size={16} /> Điểm dịch vụ (Credits)
                </div>
              </div>
              <div style={{ fontSize: 28, fontWeight: 700 }}>
                {user.credits || 0} <span style={{ fontSize: 14, fontWeight: 500, opacity: 0.8 }}>Điểm</span>
              </div>
      ...
      {currentView === "unlocked_cars" && (
        <div>
           ...
           {unlockedCarList.length === 0 ? (
             <p style={{ textAlign: 'center', color: 'var(--m-subtle)', marginTop: 40 }}>Bạn chưa dùng Điểm để mở liên hệ xe nào.</p>
           ) : (
             <div className="car-grid">
...
function TopUpModal({ user, onClose, onSave }) {
  ...
  const packages = [
    { price: 100, credits: 110, label: "Nạp 100k", bonus: "Tặng 10đ" },
    { price: 200, credits: 230, label: "Nạp 200k", bonus: "Tặng 30đ" },
    { price: 500, credits: 600, label: "Nạp 500k", bonus: "Tặng 100đ" },
  ];
  ...
  const handlePayment = async () => {
    ...
    setTimeout(async () => {
      try {
        ...
        onSave({ ...user, credits: newCredits });
        window.showAlert(`Thanh toán thành công! Bạn nhận được ${pkg.credits} Điểm.`);
        onClose();
      } catch (e) {
  ...
  return (
    <div className="modal-backdrop" style={{ zIndex: 100000 }} onClick={onClose}>
      <section className="map-modal" style={{ maxWidth: 400, padding: 24, textAlign: 'center' }} onClick={(e) => e.stopPropagation()}>
        <h2 style={{ marginTop: 0, marginBottom: 16 }}>Nạp điểm dịch vụ</h2>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
            {packages.map(pkg => (
              ...
              >
                ...
                <div style={{ color: 'var(--m-primary)', fontWeight: 600 }}>{pkg.credits} Điểm <span style={{ fontSize: 11, background: '#10b981', color: '#fff', padding: '2px 6px', borderRadius: 4, marginLeft: 4 }}>{pkg.bonus}</span></div>
              </div>
            ))}
          </div>
        </div>
...
function CommunityModal({ currentUser, onClose }) {
  ...
  const renterTips = [
    ...
    },
    {
      ...
      desc: 'Trước khi ra địa điểm nhận xe, gọi điện xác nhận lại. Nếu SĐT không thật hoặc người nghe không phải chủ xe — dừng lại.',
                ...
                <div>
                  ...
                  <div className="community-form-group">
                    ...
                    <textarea
                      ...
                      placeholder="Mô tả rõ sự việc: thời gian, địa điểm, diễn biến... Càng chi tiết càng giúp chúng tôi xử lý nhanh hơn."
                      value={description}

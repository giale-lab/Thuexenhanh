...
function App() {
  const [currentUser, setCurrentUser] = useState(() => {
    try {
      ...
      if (saved) {
        ...
        if (parsed && (parsed.avatar === GuestAvatar || parsed.avatar === "guest-avatar.png" || parsed.avatar === "/guest-avatar.png")) {
          parsed.avatar = GuestAvatar;
  ...
  useEffect(() => {
    ...
    window.deductCredits = (amount, ownerId, carId, callback) => {
      const user = currentUserRef.current;
      ...
      if ((user.credits || 0) < amount) {
        if (window.showAlert) window.showAlert(`Bạn không đủ Điểm (cần ${amount} Điểm). Vui lòng nạp thêm.`);
        return;
      }
      
      if (window.showConfirm) {
        window.showConfirm(`Sẽ trừ ${amount} Điểm để xem thông tin liên hệ. Bạn đồng ý chứ?`, async () => {
          try {
            const newCredits = (user.credits || 0) - amount;
            const updatedUnlocked = [...(user.unlockedCars || []), carId];
            await updateDoc(doc(db, "users", user.uid), { credits: newCredits, unlockedCars: updatedUnlocked });
            setCurrentUser(prev => ({ ...prev, credits: newCredits, unlockedCars: updatedUnlocked }));
            window.unlockedCars.add(carId);
            callback();
          } catch(err) {
            window.showAlert("Lỗi khi trừ điểm: " + err.message);
          }
        });
      }
    };
  ...
  const handleSaveCar = async (car) => {
    try {
      ...
      if (isNewCar) {
        ...
        if (freePosts <= 0 && (currentUser?.credits || 0) < 40) {
          window.showAlert("Số dư điểm của bạn không đủ để đăng bài (Cần 40 điểm). Vui lòng nạp thêm điểm trong phần Tài khoản.");
          return;
        }
      }
      ...
      if (window.location.protocol === 'file:' || currentUser?.isGuest) {
        ...
        if (isNewCar && currentUser) {
          ...
          if (freePosts > 0) {
            ...
            const newCredits = (currentUser.credits || 0) - 40;
            await updateDoc(doc(db, "users", currentUser.uid), { credits: newCredits });
            setCurrentUser(prev => ({ ...prev, credits: newCredits }));
            window.showAlert("Đã đăng xe thành công! Tài khoản bị trừ 40 Điểm dịch vụ.");
          }
        } else {
      ...
      <div style={{ padding: '40px 20px', maxWidth: 1000, margin: '0 auto', width: '100%' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))', gap: 24 }}>
          ...
          <div style={{ background: '#fff', padding: 24, borderRadius: 16, border: '1px solid var(--m-border)', textAlign: 'center', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 16 }}>
            ...
            <p style={{ margin: 0, fontSize: 14, color: 'var(--m-subtle)', lineHeight: 1.5 }}>Chỉ cần nhập điểm đến và thời gian, trao đổi trực tiếp với chủ xe qua SĐT, chốt hợp đồng và nhận xe ngay tức thì.</p>
          </div>
        </div>
      </div>
